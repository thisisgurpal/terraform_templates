...
provider "aws" {
  region = "eu-west-2"
  profile = "enter your profile id e.g - AdministratorAccess-32420934092"
}
...
# Subnet within VPC for resource allocation, in availability zone eu-west-2a
...
# Another subnet for redundancy, in availability zone eu-west-2b
...
resource "aws_instance" "web" {
  ...
  user_data = <<-EOF
    ...
    # Authenticate to ECR
    docker login -u AWS -p $(aws ecr get-login-password --region eu-west-2) 00000000000.dkr.ecr.eu-west-2.amazonaws.com/project_name

    # Pull the Docker image from ECR
    docker pull 00000000000.dkr.ecr.eu-west-2.amazonaws.com/project_name:latest

    # Run the Docker image
    docker run -d -p 80:8080 \
    --env SECRET_KEY=${var.secret_key} \
    --env DB_NAME=${var.db_name} \
    --env DB_USER_NM=${var.db_username} \
    --env DB_USER_PW=${var.db_password} \
    --env DB_IP=${aws_db_instance.default.address} \
    --env DB_PORT=${var.db_port} \
    00000000000.dkr.ecr.eu-west-2.amazonaws.com/project_name:latest
    EOF
